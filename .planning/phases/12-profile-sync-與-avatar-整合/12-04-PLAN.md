...
Output: 修改 includes/admin/views/settings-page.php
</objective>
...
**1. 在類別中新增 register_ajax_handlers() 方法（若不存在）：**

在 __construct() 或 init() 方法中註冊 AJAX action：
```php
...
- 呼叫 AvatarService::clearAllAvatarCache() 執行清除
  </action>
  <verify>
```bash
