...
must_haves:
  truths:
    ...
    - "State 驗證失敗時 OAuth callback 會拒絕請求並記錄日誌"
  artifacts:
  ...
  key_links:
    ...
    - from: "includes/handlers/class-login-handler.php"
      ...
      via: "authorize 時呼叫 store_state()，callback 時呼叫 verify_state() 和 consume_state()"
      pattern: "state_manager->store_state|state_manager->verify_state|state_manager->consume_state"
---
...
3. 注入 LoginService、LineUserService、StateManager 依賴
...
3. `handle_authorize()` - 處理初始授權請求
   ...
   - 產生 state 並透過 StateManager->store_state() 儲存
   - 呼叫 LoginService->get_authorize_url()
   - wp_redirect() 到 LINE
   - exit
4. `handle_callback($code, $state)` - 處理 OAuth callback
   - **首先透過 StateManager->verify_state() 驗證 state**
   - **若 state 驗證失敗: 記錄日誌並 wp_die() 顯示錯誤**
   ...
   - **最後透過 StateManager->consume_state() 消費 state（防重放攻擊）**

StateManager 整合位置：
- **handle_authorize()**:
  - `$state = $this->state_manager->generate_state();`
  - `$this->state_manager->store_state($state, ['redirect_url' => $redirect_to]);`
- **handle_callback()**:
  - `$state_data = $this->state_manager->verify_state($state);`
  - 若 `$state_data === false`: 記錄 Logger 錯誤並 wp_die('State 驗證失敗')
  - 流程結束時: `$this->state_manager->consume_state($state);`
...
class Login_Handler {
    ...
    private function handle_authorize() {
        ...
        // 取得授權 URL 並導向
        $authorize_url = $this->login_service->get_authorize_url($state);
        wp_redirect($authorize_url);
        exit;
    }

    private function handle_callback($code, $state) {
        // 驗證 state（StateManager 整合）
        $state_data = $this->state_manager->verify_state($state);
        if ($state_data === false) {
            Logger::get_instance()->log('error', [
            ...
            wp_die('State 驗證失敗，請重新嘗試登入', 'State Error', ['response' => 400]);
        }

        // ... 後續處理（exchange token, get profile, login/register）
...
```
  ...
  <verify>grep -n "class Login_Handler" includes/handlers/class-login-handler.php && grep -n "handle_login_init" includes/handlers/class-login-handler.php && grep -n "state_manager" includes/handlers/class-login-handler.php</verify>
  <done>Login_Handler 類別存在,包含 register_hooks()、handle_login_init()、handle_authorize()、handle_callback() 方法,且正確整合 StateManager（store_state/verify_state/consume_state）</done>
</task>
...
# 確認 StateManager 整合
grep -n "state_manager->store_state\|state_manager->verify_state\|state_manager->consume_state" includes/handlers/class-login-handler.php
```
...
3. Login_Handler 正確整合 StateManager（store_state/verify_state/consume_state）
4. State 驗證失敗時會拒絕請求並記錄日誌
5. LoginService callback URL 已更新為 wp-login.php?loginSocial=buygo-line
6. PHP 語法無錯誤
7. 已綁定用戶可透過 LINE Login 成功登入（Phase 9 整合後測試）
</success_criteria>
