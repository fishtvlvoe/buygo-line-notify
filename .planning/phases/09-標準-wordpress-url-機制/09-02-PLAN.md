...
需標記 deprecated 的位置共 5 處：class docblock、register_routes、authorize、callback、bind
  </action>
  <verify>grep -c "@deprecated 2.0.0" includes/api/class-login-api.php | xargs test 5 -eq && grep -n "X-BuyGo-Deprecated" includes/api/class-login-api.php</verify>
  <done>Login_API 類別和方法已標記 @deprecated（共 5 處：class、register_routes、authorize、callback、bind）,authorize() 和 callback() 會發出 deprecated header</done>
</task>
...
3. **測試未綁定用戶流程**
   - 使用未綁定的 LINE 帳號
   - 訪問: `https://test.buygo.me/wp-login.php?loginSocial=buygo-line`
   - 預期: LINE 授權後,NSLContinuePageRenderException 被捕捉,wp-login.php 正常渲染（不應 wp_die）
   - 目前預期行為: 頁面繼續渲染顯示標準 WordPress 登入表單（因為 NSLContinuePageRenderException 被捕捉但尚未有對應的 shortcode 處理，Phase 10 會實作）
   - 重要: 不應該看到錯誤頁面或 wp_die 輸出
...
```bash
# 確認 deprecated 標記（應有 5 處：class、register_routes、authorize、callback、bind）
grep -c "@deprecated 2.0.0" includes/api/class-login-api.php | xargs test 5 -eq && echo "PASS: 5 deprecated markers"
...
2. Login_API 所有端點已標記 @deprecated（共 5 處）
...
6. 未綁定用戶流程: NSLContinuePageRenderException 被捕捉,wp-login.php 正常渲染
</success_criteria>
