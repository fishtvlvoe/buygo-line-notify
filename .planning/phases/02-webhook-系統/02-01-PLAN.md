...
2. **handle_webhook() 方法**
   ...
   - 檢查 Verify Event（replyToken === '00000000000000000000000000000000'）
   - 背景處理策略：
     - FastCGI 環境：使用 shutdown hook + fastcgi_finish_request()
     - 非 FastCGI：使用 wp_schedule_single_event()
...
3. **verify_signature() 方法**
   ...
   - 開發環境（WP_DEBUG）且無 secret 時可選擇跳過驗證

4. **is_development_mode() 方法**
   - 檢查 WP_DEBUG
   - 檢查 wp_get_environment_type()（WordPress 5.5+）
   - 檢查 SERVER_NAME（localhost 等）
  </action>
  <verify>
```bash
...
- handle_webhook() 支援 FastCGI 背景處理
  </done>
</task>
