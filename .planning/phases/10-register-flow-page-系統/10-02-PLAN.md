...
2. 新增 `handle_register_submission()` 方法（包含錯誤時清除 Transient）：
```php
/**
 * 處理註冊表單提交
 *
...
private function handle_register_submission(): void {
    ...
    if (empty($user_login)) {
        ...
        wp_die('請填寫用戶名', 'Error', ['response' => 400, 'back_link' => true]);
    }

    if (empty($user_email) || !is_email($user_email)) {
        // 不清除 Transient，允許用戶修正後重試
        wp_die('請輸入有效的 Email 地址', 'Error', ['response' => 400, 'back_link' => true]);
    }
...
3. 新增 `handle_auto_link()` 方法（包含錯誤時清除 Transient）：
```php
/**
...
**Transient 清除策略說明（10 分鐘過期機制容許度）：**
- **安全驗證失敗（CSRF）：** 不清除（可能是攻擊，不要幫忙清理）
- **Transient 已過期：** 無需清除（已自動過期）
- **用戶名/Email 格式錯誤：** 不清除（允許用戶返回修正重試）
- **LINE UID 不一致：** 清除（可能是惡意篡改）
- **LINE UID 已綁定其他用戶：** 清除（無法繼續）
- **用戶已綁定其他 LINE：** 清除（無法繼續）
- **wp_insert_user 失敗：** 清除（避免數據不一致）
- **linkUser 失敗（在已建立用戶後）：** 不清除但繼續（用戶已建立）
- **成功完成：** 清除

即使某些錯誤情況下未清除 Transient，10 分鐘的自動過期機制也可容許這些情況，不會造成數據殘留問題。
  </action>
  <verify>
...
- `delete_transient` 呼叫（在多個錯誤分支和成功路徑中）

語法檢查：`php -l includes/handlers/class-login-handler.php`
  </verify>
  <done>handle_register_submission() 完整實作，包含用戶建立、LINE 綁定、Auto-link、自動登入，並在錯誤情況下正確清除 Transient</done>
</task>
...
public static function linkUser(int $user_id, string $line_uid, bool $is_registration = false): bool {
    ...
    // 明確建立 format array，避免 spread operator 相容性問題
...
**重要：** 使用明確的條件處理來建立 `$formats` 陣列，而非使用 spread operator（`...`）語法。這可確保在所有 PHP 環境中的相容性。
  </action>
  <verify>
- `LineUserService::linkUser()` 方法存在
- 方法支援 $is_registration 參數
- format array 使用明確條件處理（非 spread operator）
- 語法檢查通過：`php -l includes/services/class-line-user-service.php`
  </verify>
  <done>LineUserService.linkUser 方法與表單提交處理完全相容，且使用跨環境相容的 format array 處理方式</done>
</task>
...
- Transient 在成功和大部分錯誤情況下被正確清除
- 10 分鐘過期機制可容許少數未清除的 Transient
</success_criteria>
