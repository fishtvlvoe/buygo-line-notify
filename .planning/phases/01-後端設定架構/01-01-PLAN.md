...
4. PHP 語法檢查：`php -l includes/schemas/class-settings-schema.php`
  </verify>
  <done>
...
2. **set_group(string $group, array $data): bool|WP_Error**
   - 使用 Settings_Schema::validate($group, $data) 驗證資料
   - 驗證失敗回傳 WP_Error
...
3. 向後相容：`grep "public static function get\(" includes/services/class-settings-service.php`（原方法保留）
4. PHP 語法檢查：`php -l includes/services/class-settings-service.php`
  </verify>
  <done>
SettingsService 擴展完成：
- get_group() 可讀取群組化設定（含預設值合併）
- set_group() 可驗證並儲存群組化設定
...
<task type="auto">
  <name>Task 3: 註冊 Schema 類別到 Plugin 載入流程</name>
  <files>includes/class-plugin.php</files>
  <action>
更新 Plugin 類別，確保 Settings_Schema 在 SettingsService 之前載入。
...
2. 如果有 index.php 需要建立，在 includes/schemas/ 目錄建立：
   ```php
   <?php
   // Silence is golden.
   ```

注意：不要修改其他載入邏輯，僅新增 schema 檔案的載入。
  </action>
  <verify>
1. Schema 載入：`grep "settings-schema" includes/class-plugin.php`
2. 目錄索引存在：`ls includes/schemas/index.php`
...
3. **功能驗證（WordPress CLI 或測試腳本）**
   建立測試腳本驗證：
   - Settings_Schema::get_schema() 回傳正確結構
   - Settings_Schema::validate() 正確驗證資料
   - SettingsService::get_group() 回傳含預設值的設定
   - SettingsService::set_group() 正確儲存設定
</verification>
...
5. [ ] SettingsService 新增 get_group/set_group/get_all_groups 方法
6. [ ] 敏感欄位自動加密/解密
7. [ ] 向後相容：原有 get/set 方法不受影響
8. [ ] 所有 PHP 檔案語法正確
</success_criteria>
