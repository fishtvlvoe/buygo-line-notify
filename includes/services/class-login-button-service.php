...
class LoginButtonService {
	...
	public static function register_hooks() {
		...
		add_action( 'fluent_community/before_auth_form_header', [ __CLASS__, 'render_fluent_community_button' ], 10, 1 );

		// Ajax Login and Registration Modal Popup Pro
		add_action( 'lrm/login_form/before', [ __CLASS__, 'render_lrm_button' ] );

		// WordPress 原生登入頁面
		add_action( 'login_form', [ __CLASS__, 'render_wp_login_button' ] );
	}
	...
	private static function render_button( $context ) {
		...
		$button_text  = apply_filters( 'buygo_line_notify/login_button/text', __( '使用 LINE 登入', 'buygo-line-notify' ), $context );

		?>
		<div class="buygo-line-login-wrapper <?php echo esc_attr( "buygo-line-login-{$context}" ); ?>" style="margin-bottom: 20px;">
			<button type="button"
			        class="<?php echo esc_attr( $button_class ); ?>"
			        id="buygo-line-login-btn"
			        data-api-url="<?php echo esc_url( $api_url ); ?>"
			        style="width: 100%; padding: 12px; font-size: 16px; border: none; border-radius: 4px; cursor: pointer; background-color: #06C755; color: white; display: flex; align-items: center; justify-content: center; gap: 8px;">
			...
			<span class="buygo-line-login-loading" style="display:none; margin-left: 10px; font-size: 14px; color: #666;">載入中...</span>
		</div>

		<script>
		(function() {
			var btn = document.getElementById('buygo-line-login-btn');
			if (!btn) return;

			btn.addEventListener('click', function() {
				var apiUrl = this.getAttribute('data-api-url');
				var loading = document.querySelector('.buygo-line-login-loading');
